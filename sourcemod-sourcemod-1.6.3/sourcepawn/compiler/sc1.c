...
int mod_debug_options = 0;
int mod_no_debugger = 0;
...
static void initglobals(void)
{
  ...
  mod_debug_options=0;		/* modification */
                         ...
                         char *rname, char *codepage)
{
  ...
  for (arg=1; arg<argc; arg++) {
    ...
    if (isoption) {
	  ...
	  case 'b':
		  /*
		  * 0 - Allow 'break' opcodes for debuggers
		  * 1 - Remove 'break' opcodes
		  * 2 - Randomize 'break' opcodes
		  */
		switch (*option_value(ptr)) {
		  case '0':
			  mod_no_debugger = 0;
	  ...
	  case 'z':
			/*
			* 0 - no manipulation
			* 1 - debug section removal
			* 2 - debug-symbol randomization
			*/
        switch (*option_value(ptr)) {
		 case '0':
			mod_debug_options = 0;
...
static void setcaption(void)
{
	pc_printf("Modified SourcePawn Compiler %s\n", "1.6.3"/*SOURCEMOD_VERSION*/);
	pc_printf("Copyright (c) 1997-2006, ITB CompuPhase, (C)2004-2008 AlliedModders, LLC\n");
	pc_printf("Modified by Externet (aka Timocop)\n\n");
}
...
static void about(void)
{
  if (strlen(errfname)==0) {
	...
	pc_printf("Modification options:\n");
	pc_printf("         -z<num>  Debug section options\n");
	pc_printf("             0    no changes\n");
	pc_printf("             1    remove debug sections\n");
	pc_printf("             2    debug symbols randomization\n");

	pc_printf("         -b<num>  Debugger breakpoint options\n");
	pc_printf("             0    no changes\n");
	pc_printf("             1    remove debugger breakpoints\n");
	pc_printf("             2    debugger breakpoint randomization\n");
  } /* if */
  norun = 1;
